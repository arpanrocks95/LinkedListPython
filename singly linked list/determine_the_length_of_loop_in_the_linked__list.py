...
class linkedList:
    ...
    def detectLengthOfLoopInALinkedList(self,len):

        print(len)
        temp = self.head
        fasttemp = self.head
        lenFrom1STTOStartnodeofloop = 0;
        desc = 0
        while temp and fasttemp and fasttemp.next:
            if desc == 0:
                ...
                lenFrom1STTOStartnodeofloop = lenFrom1STTOStartnodeofloop+1
            if temp == fasttemp:
                if desc == 0:
                    temp = self.head
                    desc = 1
                    ...
                    print("loop detected ")
                    return len-lenFrom1STTOStartnodeofloop
...
if __name__=='__main__':
    ...
    ll.insertAtStart(15)
    ll.display()
    len = ll.LengthLinkedListIterative()
    ...
    #now detecting the length of  loop
    print("The length Of the Loop Is: {0}".format(ll.detectLengthOfLoopInALinkedList(len)))